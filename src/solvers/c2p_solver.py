...
class C2P_Solver:
    ...
    def normalize_C(self, C):
        """Min-max normalize C using stored min/max."""
        return (C - self.C_min) / (self.C_max - self.C_min + 1e-12)

    def denormalize_C(self, C_norm):
        """Reverse min-max normalization on C."""
        return C_norm * (self.C_max - self.C_min) + self.C_min

    def normalize_Z(self, Z):
        """Min-max normalize Z using stored min/max."""
        return (Z - self.Z_min) / (self.Z_max - self.Z_min + 1e-12)

    def denormalize_Z(self, Z_norm):
        """Reverse min-max normalization on Z."""
        return Z_norm * (self.Z_max - self.Z_min) + self.Z_min
