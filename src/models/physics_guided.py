...
from src.physics.hybrid_eos import hybrid_eos
from src.physics.physics_utils import W__z, rho__z, eps__z, h__z, a__z, sanity_check
from src.solvers.c2p_solver import C2P_Solver
...
class PhysicsGuided_TinyPINNvKen(nn.Module):
    ...
    def __init__(self, dtype=torch.float64, correction_scale=0.2):
        ...
        print("Starting PhysicsGuided_TinyPINNvKen")
    ...
    #
        """
        Quadratic polynomial on normalized inputs.
        MAE: 3.32e-04 (0.033%), R²: 0.999991
        """
        D = C[:, 0:1]
        q = C[:, 1:2]
        r = C[:, 2:3]

        # Quadratic polynomial fitted to training data
        Z_norm = 8.157327e-04 + \
                -4.042044e-01 * D + \
                1.114623e+00 * q + \
                1.400320e+00 * r + \
                -1.002859e-01 * torch.square(D) + \
                5.815399e-01 * (D * q) + \
                -7.191322e-01 * (D * r) + \
                5.085327e-01 * torch.square(q) + \
                4.971266e-02 * (q * r) + \
                -1.436708e+00 * torch.square(r)

        return torch.clamp(Z_norm, min=0.0)
